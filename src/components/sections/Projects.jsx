        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {projectsData.projects.map((project, index) => (
              <div
                key={index}
                className="bg-gray-800/50 p-6 rounded-xl border border-gray-700 shadow-lg hover:shadow-purple-500/20 transition-all duration-300 transform hover:-translate-y-1 flex flex-col"
              >
                {project.thumbnail && (
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {project.tech.map((tech, key) => {
                    ...
                    return (
                      <span
                        key={key}
                        className={`py-1 px-3 rounded-full text-xs font-medium ${isBackend ? 'bg-purple-600 text-white' : 'bg-gray-700 text-gray-200'} transition-colors duration-200`}
                      >
                        {tech}
                      </span>
                    );
                ...
                <div className="flex flex-wrap gap-3 mt-auto pt-4 border-t border-gray-700">
                  ...
                  {project.backendLink && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="px-4 py-2 bg-purple-700 hover:bg-purple-600 text-white text-sm rounded-full transition-colors duration-200 flex items-center space-x-1"
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
